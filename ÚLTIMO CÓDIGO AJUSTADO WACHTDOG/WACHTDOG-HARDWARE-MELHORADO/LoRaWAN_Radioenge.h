...
enum{
  ...
  RADIOENGE_OUTPUT_PUSH_PULL,
  RADIOENGE_OUTPUT_OPENDRAIN,
  RADIOENGE_OUTPUT_FA_PUSHPULL,
  RADIOENGE_OUTPUT_FA_OPENDRAIN,

  RADIOENGE_INPUT_ADC,
  RADIOENGE_INTERRUPT_RISING,
  RADIOENGE_INTERRUPT_FALLING,
  RADIOENGE_INTERRUPT_CHANGE,
  N_RADIOENGE_TYPES_GPIO
};
...
class LoRaWAN_Radioenge{
  private:
    ...
    String commandAT(uint8_t cmd, String val = "", bool exception = false){
      ...
      // strlen espera char*, por isso o cast
      for(uint8_t lk = 0; lk < (uint8_t)strlen((const char*)AT_CMD[cmd]); lk++)
      {
  ...
  public:
    ...
    void begin(){
      ...
      // Habilita ADR por padrão ao iniciar a interface (comportamento solicitado)
      ADR(true);
    }
    ...
    // --- Getter: lê o DR atual (NÃO altera nada)
    uint8_t DR(){
      return (uint8_t)commandAT(_DR_).toInt();
    }

    // --- Setter: define o DR explicitamente quando necessário
    // retorna true se o comando foi enviado (valor válido), false caso contrário
    bool setDR(uint8_t val){
      if(val < 14){
        commandAT(_DR_, String(val));
        return true;
      }
      return false;
    }
    ...
    bool pinMode(uint8_t pin, uint8_t modo, uint8_t pull = RADIOENGE_NO_PULL){
      
      if((pin > 9) || (modo >= N_RADIOENGE_TYPES_GPIO) || (pull >= N_RADIOENGE_PULL)) return false;

      else if(((modo == RADIOENGE_OUTPUT_FA_PUSHPULL) || (modo == RADIOENGE_OUTPUT_FA_OPENDRAIN)) && pin != 0 && pin != 1) return false;     
      else if((modo == RADIOENGE_INPUT_ADC) && (pin != 0) && (pin != 1) && (pin != 7) && (pin != 8)) return false; 
      else if(((modo == RADIOENGE_INTERRUPT_RISING) || (modo == RADIOENGE_INTERRUPT_FALLING) || (modo == RADIOENGE_INTERRUPT_CHANGE)) && (pin == 0) && (pin == 3) && (pin == 7) && (pin == 8)) return false;
    ...
    void ConfigNetwork(uint8_t njm = NULL, uint8_t net = NULL, String appkey = "", String appeui = "", String nwkskey = "", String daddr = ""){
      ...
      // Não forçamos DR aqui para evitar sobrescrever configurações explicitamente definidas em runtime.
    }
    ...
    // --- Função pública para controlar ADR em runtime
